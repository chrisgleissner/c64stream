                                        ...
                                        size_t buffer_size)
{
    ...
    obs_log(LOG_DEBUG, "[c64stream] Direct DNS query to %s for hostname: %s", dns_server, hostname);

    // Initialize resolver
    if (res_ninit(&res) != 0) {
        obs_log(LOG_DEBUG, "[c64stream] res_ninit failed for DNS server %s", dns_server);
        return false;
    }
    ...
    if (inet_aton(dns_server, &dns_addr.sin_addr) == 0) {
        obs_log(LOG_DEBUG, "[c64stream] Invalid DNS server IP: %s", dns_server);
        res_nclose(&res);
        return false;
    }
    ...
    if (len < 0) {
        obs_log(LOG_DEBUG, "[c64stream] Direct DNS query failed for %s via %s (h_errno: %d)", hostname, dns_server, h_errno);
        res_nclose(&res);
        return false;
    }
    ...
    if (ns_initparse(answer, len, &msg) < 0) {
        obs_log(LOG_DEBUG, "[c64stream] DNS response parsing failed for %s", hostname);
        res_nclose(&res);
        return false;
    }
    ...
    if (ancount == 0) {
        obs_log(LOG_DEBUG, "[c64stream] No A record found for %s via %s", hostname, dns_server);
        res_nclose(&res);
        return false;
    }
    ...
    for (int i = 0; i < ancount; i++) {
        ...
        if (ns_rr_type(rr) == ns_t_a && ns_rr_rdlen(rr) == 4) {
            ...
            if (strlen(ip_str) < buffer_size) {
                ...
                obs_log(LOG_INFO, "[c64stream] Direct DNS resolved %s -> %s (via %s)", hostname, ip_str, dns_server);
                res_nclose(&res);
                return true;
            }
        }
                                               ...
                                               size_t buffer_size)
{
    ...
    if (custom_dns && strlen(custom_dns) > 0) {
        ...
        obs_log(LOG_DEBUG, "[c64stream] Using configured DNS server: %s", custom_dns);
    }
...
bool c64_init_networking(void)
{
    ...
    if (result != 0) {
        obs_log(LOG_ERROR, "[c64stream] WSAStartup failed: %d", result);
        return false;
    }
    obs_log(LOG_DEBUG, "[c64stream] Windows networking initialized");
#endif
    return true;
}
...
bool c64_detect_local_ip(char *ip_buffer, size_t buffer_size)
{
            ...
            if (ret_val == NO_ERROR) {
                ...
                while (adapter) {
                    ...
                    if (adapter->Type != MIB_IF_TYPE_LOOPBACK &&
                        ...
                        ip_buffer[buffer_size - 1] = '\0';
                        obs_log(LOG_INFO, "[c64stream] Detected Windows IP address: %s (adapter: %s)", ip_buffer,
                                adapter->AdapterName);
                        free(adapter_info);
                        return true;
                    }
    ...
    obs_log(LOG_WARNING, "[c64stream] Failed to detect Windows IP address, using fallback");
    return false;
    ...
    if (getifaddrs(&ifaddrs_ptr) == -1) {
        obs_log(LOG_WARNING, "[c64stream] getifaddrs failed: %s", strerror(errno));
        return false;
    }
    ...
    for (ifa = ifaddrs_ptr; ifa != NULL; ifa = ifa->ifa_next) {
        ...
        ip_buffer[buffer_size - 1] = '\0';
        obs_log(LOG_INFO, "[c64stream] Detected %s IP address: %s (interface: %s)",
#ifdef __APPLE__
                "macOS",
#else
                "Linux",
    ...
    freeifaddrs(ifaddrs_ptr);
    obs_log(LOG_WARNING, "[c64stream] No suitable network interface found, using fallback");
    return false;

#else
    // Fallback for other platforms
    obs_log(LOG_WARNING, "[c64stream] IP detection not implemented for this platform, using fallback");
    return false;
#endif
}
...
#ifdef _WIN32
    WSACleanup();
    obs_log(LOG_DEBUG, "[c64stream] Windows networking cleaned up");
#endif
}
                                   ...
                                   size_t buffer_size)
{
    ...
    if (inet_pton(AF_INET, hostname, &sa.sin_addr) == 1) {
        ...
        ip_buffer[buffer_size - 1] = '\0';
        obs_log(LOG_DEBUG, "[c64stream] Input '%s' is already an IP address", hostname);
        return true;
    }

    obs_log(LOG_DEBUG, "[c64stream] Attempting to resolve hostname: %s", hostname);
    ...
    if (status == 0 && result != NULL) {
        ...
        if (inet_ntop(AF_INET, &addr_in->sin_addr, ip_buffer, buffer_size) != NULL) {
            obs_log(LOG_INFO, "[c64stream] System DNS resolved '%s' to IP: %s", hostname, ip_buffer);
            freeaddrinfo(result);
            return true;
        }
    }
    ...
    obs_log(LOG_DEBUG, "[c64stream] Trying FQDN resolution: %s", hostname_with_dot);

    status = getaddrinfo(hostname_with_dot, NULL, &hints, &result);
    if (status == 0 && result != NULL) {
        struct sockaddr_in *addr_in = (struct sockaddr_in *)result->ai_addr;
        if (inet_ntop(AF_INET, &addr_in->sin_addr, ip_buffer, buffer_size) != NULL) {
            obs_log(LOG_INFO, "[c64stream] FQDN resolved '%s' to IP: %s", hostname_with_dot, ip_buffer);
            freeaddrinfo(result);
            return true;
        }
    }
    ...
    obs_log(LOG_DEBUG, "[c64stream] System DNS failed, trying direct DNS server queries");
    ...
    obs_log(LOG_WARNING, "[c64stream] Failed to resolve hostname '%s' using all available methods", hostname);
    return false;
}
...
socket_t c64_create_udp_socket(uint32_t port)
{
    ...
    if (sock == INVALID_SOCKET_VALUE) {
        ...
        obs_log(LOG_ERROR, "[c64stream] Failed to create UDP socket: %s", c64_get_socket_error_string(error));
    ...
    if (setsockopt(sock, SOL_SOCKET, SO_RCVBUF, (char *)&recv_buffer_size, sizeof(recv_buffer_size)) < 0) {
        int error = c64_get_socket_error();
        obs_log(LOG_WARNING, "[c64stream] Failed to set UDP receive buffer size to %d bytes: %s", recv_buffer_size,
                c64_get_socket_error_string(error));
    } else {
        obs_log(LOG_DEBUG, "[c64stream] Set UDP receive buffer to %d bytes for high-frequency packet handling",
                recv_buffer_size);
    }
    ...
    if (setsockopt(sock, IPPROTO_UDP, UDP_NOCHECKSUM, (char *)&udp_nochecksum, sizeof(udp_nochecksum)) < 0) {
        ...
        obs_log(LOG_DEBUG, "[c64stream] UDP_NOCHECKSUM option not supported on this system");
    }
#else
    ...
    if (setsockopt(sock, SOL_SOCKET, SO_RCVBUF, &recv_buffer_size, sizeof(recv_buffer_size)) < 0) {
        ...
        obs_log(LOG_DEBUG, "[c64stream] Set UDP receive buffer to %d bytes", recv_buffer_size);
    }
#endif
    ...
    if (bind(sock, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
        int error = c64_get_socket_error();
        obs_log(LOG_ERROR, "[c64stream] Failed to bind UDP socket to port %u: %s", port, c64_get_socket_error_string(error));
        close(sock);
        return INVALID_SOCKET_VALUE;
    }
    ...
    if (ioctlsocket(sock, FIONBIO, &mode) != 0) {
        int error = c64_get_socket_error();
        obs_log(LOG_WARNING, "[c64stream] Failed to set socket non-blocking: %s", c64_get_socket_error_string(error));
    }
#else
    ...
    obs_log(LOG_INFO, "[c64stream] Created optimized UDP socket on port %u", port);
...
{
    if (!ip || strlen(ip) == 0) {
        obs_log(LOG_ERROR, "[c64stream] Invalid IP address provided");
        return INVALID_SOCKET_VALUE;
    }
    ...
    if (sock == INVALID_SOCKET_VALUE) {
        int error = c64_get_socket_error();
        obs_log(LOG_ERROR, "[c64stream] Failed to create TCP socket: %s", c64_get_socket_error_string(error));
        return INVALID_SOCKET_VALUE;
    }
    ...
    if (inet_pton(AF_INET, ip, &addr.sin_addr) <= 0) {
        obs_log(LOG_ERROR, "[c64stream] Invalid IP address format: %s", ip);
        close(sock);
        return INVALID_SOCKET_VALUE;
    }
    ...
    u_long non_blocking = 1;
    if (ioctlsocket(sock, FIONBIO, &non_blocking) != 0) {
        obs_log(LOG_ERROR, "[c64stream] Failed to set socket non-blocking");
        close(sock);
        return INVALID_SOCKET_VALUE;
    }
#else
...
#endif
        obs_log(LOG_DEBUG, "[c64stream] Connected to C64 Ultimate at %s:%u", ip, port);
        return sock;
    }
    ...
    if (error != EINPROGRESS) {
#endif
        obs_log(LOG_WARNING, "[c64stream] Failed to connect to C64 Ultimate at %s:%u: %s", ip, port,
                c64_get_socket_error_string(error));
        close(sock);
        return INVALID_SOCKET_VALUE;
    }
    ...
    if (select_result == 0) {
        ...
        obs_log(LOG_DEBUG, "[c64stream] Fast connection attempt to %s:%u timed out, trying slower timeout...", ip, port);
        ...
        if (select_result == 0) {
            ...
            obs_log(LOG_WARNING, "[c64stream] Connection to C64 Ultimate at %s:%u timed out after 1.6 seconds total", ip,
                    port);
            close(sock);
            return INVALID_SOCKET_VALUE;
        }
    ...
    if (select_result < 0) {
        ...
        obs_log(LOG_ERROR, "[c64stream] Select failed during connection to %s:%u: %s", ip, port,
    ...
    if (getsockopt(sock, SOL_SOCKET, SO_ERROR, &sock_error, &len) < 0) {
#endif
        obs_log(LOG_ERROR, "[c64stream] Failed to get socket error for %s:%u", ip, port);
        close(sock);
        return INVALID_SOCKET_VALUE;
    }
