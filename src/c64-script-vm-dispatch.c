                                ...
                                bool *skip_line_update)
{
    ...
    case OP_CFG_SET: {
        ...
        bool ok = set_config_value(runtime, category.as.string, item.as.string, value.as.string, "CFG");
        c64script_value_free(&value);
        c64script_value_free(&item);
        c64script_value_free(&category);
        if (!ok) {
    ...
    case OP_SID_MODEL: {
        ...
        bool ok = set_config_value(runtime, category, item, model.as.string, "SID_MODEL");
        c64script_value_free(&model);
        c64script_value_free(&target);
        if (!ok) {
            return false;
    ...
    case OP_SID_ENABLE: {
        ...
        bool ok = set_config_value(runtime, category, item, value, "SID_ENABLE");
        c64script_value_free(&enabled);
        c64script_value_free(&target);
        if (!ok) {
            return false;
    ...
    case OP_SID_VOL: {
        ...
        bool ok = set_config_value(runtime, category, item, level.as.string, "SID_VOL");
        c64script_value_free(&level);
        c64script_value_free(&target);
        if (!ok) {
            return false;
    ...
    case OP_SID_FILTER_CURVE: {
        ...
        bool ok = set_config_value(runtime, category, item, curve.as.string, "SID_FILTER_CURVE");
        c64script_value_free(&curve);
        c64script_value_free(&target);
        if (!ok) {
            return false;
    ...
    case OP_SID_RESONANCE: {
        ...
        bool ok = set_config_value(runtime, category, item, resonance.as.string, "SID_RESONANCE");
        c64script_value_free(&resonance);
        c64script_value_free(&target);
        if (!ok) {
            return false;
    ...
    case OP_SID_COMBINED: {
        ...
        bool ok = set_config_value(runtime, category, item, combined.as.string, "SID_COMBINED");
        c64script_value_free(&combined);
        c64script_value_free(&target);
        if (!ok) {
            return false;
    ...
    case OP_SID_DIGIS: {
        ...
        bool ok = set_config_value(runtime, category, item, level.as.string, "SID_DIGIS");
        c64script_value_free(&level);
        c64script_value_free(&target);
        if (!ok) {
            return false;
    ...
    case OP_VIC_MODE: {
        ...
        bool ok = set_config_value(runtime, "U64 Specific Settings", "System Mode", mode.as.string, "VIC_MODE");
        c64script_value_free(&mode);
        if (!ok) {
            return false;
        }
    ...
    case OP_CPU_SPEED: {
        ...
        bool ok = set_config_value(runtime, "U64 Specific Settings", "CPU Speed", speed.as.string, "CPU_SPEED");
        c64script_value_free(&speed);
        if (!ok) {
            return false;
        }
    ...
    case OP_DRIVE_BUS_ID: {
        ...
        bool ok = set_config_value(runtime, category, "Drive Bus ID", value_buf, "DRIVE_BUS_ID");
        c64script_value_free(&bus_id);
        c64script_value_free(&drive_val);
        if (!ok) {
            return false;
    ...
    case OP_READFILE: {
        // READFILE path, variable - Read file content into variable
    ...
    case OP_WRITEFILE_TRUNCATE: {
        ...
        char err[256] = {0};
        if (!write_file(path_val.as.string, content_str, truncate, err, sizeof(err))) {
            c64script_value_free(&path_val);
            c64script_value_free(&content_val);
