...
- **Keymap:** Select keyboard mapping for converting PC keystrokes to C64 PETSCII codes:
  ...
  - **Held keys and games:** On firmware with the `machine:input` API, a key you hold stays pressed on the C64 for as long as you hold it (rather than a brief tap), so action games that read keys directly — e.g. pinball flippers — respond correctly. The two keymap types differ in how a held **Shift** reaches the C64: **positional** treats Shift as the literal C64 left/right Shift keys, so holding Left/Right Shift drives the two keyboard "flippers" that many games (e.g. *David's Midnight Magic*) use; **symbolic** consumes Shift to pick characters for typing, so it does not act as a flipper there. Choose **positional** for such games. Other keys (letters, digits, cursor keys, Space) hold identically in both modes. Ctrl and Commodore are not mirrored as held keys. On older firmware without `machine:input`, keys always fall back to a single tap and cannot be held.
